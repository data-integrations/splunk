...
public class SplunkReceiver extends Receiver<Map<String, String>> {
  ...
  /**
   * Constructor for SplunkReceiver object.
   * @param config the splunk streaming source config
  ...
  private void receive() {
    try {
      /*TO DO https://issues.cask.co/browse/PLUGIN-361
       * allRecordsCount is always zero. This count is not updated properly and needs to be fixed.
       */
      ...
      /*TO DO
       *  Check the exception code , check if retries will fix transient errors. Log retries.
       */
